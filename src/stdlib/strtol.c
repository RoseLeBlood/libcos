...
long int strtol  (const char* nptr, char** endptr, int base) {
	...
	_cutoff = _neg ? -(unsigned long)LONG_MIN : LONG_MAX;
	cutlim = (int)(_cutoff % (unsigned long)base);
	_cutoff /= (unsigned long)base;
	...
	for(result = 0, any = 0;; _char = *_ptr++) {
		...
		if(any < 0 || result > _cutoff || (result == _cutoff && _char > cutlim))
			...
			result += (unsigned long)_char;
		}
	}

	if(any < 0) result = _neg ? (unsigned long)LONG_MIN : (unsigned long)LONG_MAX;
